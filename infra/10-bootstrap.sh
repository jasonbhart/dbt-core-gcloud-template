...
ensure_bucket(){ local uri="gs://${DOCS_BUCKET_NAME}"; if gcloud storage buckets describe "$uri" --project "$PROJECT_ID" >/dev/null 2>&1; then info "Bucket exists: $uri"; else info "Creating bucket: $uri"; gcloud storage buckets create "$uri" --location="$REGION" --project "$PROJECT_ID"; fi }
